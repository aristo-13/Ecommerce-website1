 ...
 const handleNewUser = async(e) => {
  ...
  return (
    <div className='bg-[whitesmoke] w-full  h-screen overflow-hidden flex justify-center items-center'>
      ...
      <div className='pt-[7rem] border h-full sm:h-[400px] w-[min(100%,350px)] sm:pt-1 p-5 flex flex-col bg-white rounded-lg shadow-2xl'>
      <h1 className='w-full p-5 text-xl font-medium text-LightBlue'>🛒Sign Up</h1>
    <form onSubmit={handleNewUser} className='w-full flex flex-col items-center gap-3 p-3'>
        <div className='w-full h-[40px] border relative rounded-lg overflow-hidden'>
            ...
            <input className='w-full h-full px-10 outline-none' type="text" placeholder='username' onChange={(e) => setUserName(e.target.value)}/>
        </div>
        <div className='w-full h-[40px] border relative rounded-lg overflow-hidden'>
